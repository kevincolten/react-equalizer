    ...
    value: function updateChildrenHeights() {
      ...
      if (node !== undefined) {
        ...
        for (var row = 0; row < heights.length; row++) {
          var max = heights[row][heights[row].length - 2];
          if (this.props.square) {
            var maxWidth = heights[row][heights[row].length - 1];
            max = Math.max(max, maxWidth);
          }

          for (var i = 0; i < heights[row].length - 2; i++) {
    ...
    value: function getHeights(nodes) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        var _node$getBoundingClie = node.getBoundingClientRect(),
            ...
            elWidth = _node$getBoundingClie.width;
      ...
      for (var j = 0; j < groups.length; j++) {
        ...
        groups[j].push(max);
        var widths = groups[j].slice(0, -1).map(function (item) {
          return item[2];
        });
